...
description: Typed knowledge graph for structured agent memory and composable skills.
  Use when creating/querying entities (Person, Project, Task, Event, Document), linking
  related objects, enforcing constraints, planning multi-step actions as graph transformations,
  or when skills need to share state. Trigger on "remember", "what do I know about",
  "link X to Y", "show dependencies", entity CRUD, or cross-skill data access.；扩展能力：新增实体类型
  Column、Dimension、Metric、Table；关系 depends_on、derives_from、has_column、references_dimension；其中
  2 条关系声明为无环（acyclic），可用于依赖环路校验。
---
# Ontology
...
This skill has additional schema elements defined in the reference docs.

- See `references/schema.md` → Extension types (4 entries).
- See `references/schema.md` → Extension relations (4 entries).
- For example queries and usage patterns, see `references/queries.md`.
